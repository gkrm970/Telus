...
@router.post("/fed_kafka_bus/insert", status_code=201, response_model=None)
...
@router.put("/fed_kafka_bus/update/{fed_kafka_bus_id}", status_code=201, response_model=None)
...
@router.get("/fed_kafka_bus/get_by_id/{fed_kafka_bus_id}", status_code=200)
async def get_data_with_id(fed_kafka_bus_id: int, db: Session = Depends(get_db), current_user: TokenPayload = Depends(get_current_user)):
    get_fed_kafka_data_with_id = CRUDDevice.get_data_by_id(db=db, kafka_bus_info_id=fed_kafka_bus_id)
    return get_fed_kafka_data_with_id


@router.get("/fed_kafka_bus/all_records", status_code=200)
...
@router.delete("/fed_kafka_bus/delete/{fed_kafka_bus_id}")
async def delete_fed_kafka_bus_info(fed_kafka_bus_id: int, db: Session = Depends(get_db), current_user: TokenPayload = Depends(get_current_user)):
    ...
    return {"response": "Record_Deleted Successfully", "status_code": 200}
